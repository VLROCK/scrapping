...
DB_ORIGEM = "data/dataset_jornal_balanceado.db"     # Banco unificado dos 3 datasets literários
DB_DESTINO = "data/dataset_jornal_balanceado2.db"  # Banco destino para não quebrar o original
QTD_LONGO = 30
...
def fracionar_literatura_seguro():
    ...
    query = f"""
        SELECT * FROM texts 
        WHERE size_category IN ('Médio-Longo') 
        ORDER BY RANDOM() 
        LIMIT {QTD_LONGO}
    """
    ...
    for _, row in tqdm(df_alvo.iterrows(), total=len(df_alvo)):
        ...
        for pedaco in pedacos:
            ...
            if nova_categoria in ['Médio-Longo']:
                continue
