...
async def extrair_artigo(
    ...
    async with semaphore:
        ...
        url_html = API_HTML_URL.format(pid=pid)
    ...
    texto = re.split(
        r'\n(?:Referências|Referências Bibliográficas|Bibliografia|References|Referencias)\s*(?:\n|$)',
...
async def run_scielo_pipeline(total_pids: int = 200, max_concurrent: int = 10):
    ...
    async with aiohttp.ClientSession() as session:
        pids = await discovery_scielo(session, total=total_pids)
        # Remove os já visitados antes de processar
        pids = [p for p in pids if p not in seen_pids]
        print(f"[Discovery] {len(pids)} PIDs novos para processar.")

        tasks = [
            extrair_artigo(session, semaphore, hashes_lock,
                           pid, seen_hashes, seen_pids, db, lang_model)
            for pid in pids
        ]
        results = await asyncio.gather(*tasks)
        salvos  = sum(r for r in results if r)

    await db.close()
    print(f"\n[Finalizado] {salvos}/{len(pids)} artigos salvos.")


if __name__ == "__main__":
    asyncio.run(run_scielo_pipeline(total_pids=1000, max_concurrent=10))