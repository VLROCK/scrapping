...
# [ALTERADO] Banco focado apenas no lixo
DB_PATH = "data/dataset_lixo.db"

BLOCKED_URL_PATTERNS = [
    "/ao-vivo", "/index", "/categoria", "/tag/",
    "/autor/", "/busca", "/search", "/page/", "/galeria", "/login","/embed",     
    "/videos_e_fotos","/video/", "/blog/", "/platb/", "/live/", "/fotos/", "/album/", "/enquete/", "/blogs/", "/Common/"
]
...
class WaybackScraperTrashPipeline:
    ...
    async def fetch_and_process_html(self, session: aiohttp.ClientSession, snapshot: Dict):
        ...
        # [ALTERADO] Removida a barreira do BLOCKED_URL_PATTERNS. 
        # Agora nós QUEREMOS que os robôs entrem nas páginas de galeria, autor e categorias!
    ...
    # ── Filtros (Agora usados como Guilhotina de Captura) ────────────────────
    ...
    def limpar_residuos(self, texto: str) -> str:
        ...
        nav_patterns = [
            r'(?i)^publicidade\.?$', r'(?i)^compartilhar\.?$', r'(?i)^clique aqui\.?$', 
            r'(?i)^leia (também|mais)\.?$', r'(?i)^veja (também|mais)\.?$', r'(?i)^saiba mais\.?$'
        ]
    ...
    # ── [LÓGICA INVERTIDA] Avaliador de Lixo ─────────────────────────────────
    async def process_text(self, html_str: str, original_url: str, timestamp: str):
        ...
        # Lixo vazio do trafilatura não vale a pena guardar no banco
        ...
        # 2. Armadilhas de Conteúdo Direto
        ...
        if not motivo_lixo:
            ...
            if not linhas_conteudo: return
                ...
                motivo_lixo = "Poucas palavras por linha"
            elif len(texto_limpo) / (len(html_str) + 1) < 0.02:
                motivo_lixo = "Pouco texto útil vs HTML"
            elif not self.is_textual_article(texto_limpo):
                motivo_lixo = "Formato Não-Textual (Menús/Listas)"

        # 4. Armadilhas de Tamanho e Idioma
        ...
        # [O GRANDE SEGREDO] Se não for lixo, significa que o texto é excelente...
        # Como o objetivo é construir o Dataset de Ruído, textos bons são descartados!
        if not motivo_lixo:
            return
        ...
        await self.save_record({
            ...
            "content":        texto_limpo,
            "label":          1, # <--- LABEL 1 INJETADO (ANOMALIA/RUÍDO)
            "broad_area":     "Ruído/Lixo",
            "specific_theme": motivo_lixo, # <--- O MOTIVO É SALVO AQUI
    ...
    async def run(self, max_test_urls: Optional[int] = None):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                domain_tasks = [self.get_cdx_snapshots(session, d) for d in self.target_domains]
                ...
                for i in range(0, len(all_snapshots), batch_size):
                    ...
                    await asyncio.gather(*[self.fetch_and_process_html(session, s) for s in batch])

                    elapsed = time.monotonic() - self.start_time
                    print(f"[Progresso] Lote {i//batch_size + 1} | Salvos: {len(self.seen_hashes)} | Tempo: {elapsed/60:.1f}min")
        ...
        elapsed = time.monotonic() - self.start_time
        print(f"\n[Finalizado] {len(self.seen_hashes)} textos inúteis capturados em {elapsed/60:.1f} minutos.")
...
if __name__ == "__main__":
    ...
    pipeline = WaybackScraperTrashPipeline(
